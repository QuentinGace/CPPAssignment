...
double Function::Richtmyer(double Fi, double Fi1, double Fi2, double dx, double dt)
{
	return Fi*(1-(v*dt/dx))+(v/2)*Fi1*((dt/dx)-1)+(v/2)*Fi2*(1+(dt/dx)); //Fi = U(i,n) ; Fi1 = U(i+1,n) ; Fi2 = U(i-1,n)
}
